                ...
                <tbody>
                    ...
                    @foreach ($cargoItems as $item)
                        @php
                            ...
                            $measureRequired = strtolower($item->cargo_item_measure_required ?? 'no');

                            if ($measureRequired === 'yes') {
                                // Per quantity pricing
                                $subtotal = $item->freight * $item->quantity;
                                $rateDisplay = '₱' . number_format($item->freight, 2) . ' / qty';
                            } else {
                                // CBM-based pricing
                                $subtotal = $cbm * $item->freight * $item->quantity;
                                $rateDisplay = '₱' . number_format($item->freight, 2) . ' / CBM';
                            }
                        ...
                        <tr>
                            ...
                            @if($measureRequired === 'yes')
                                <td colspan="3" class="text-center text-muted">Standard Size</td>
                            @else

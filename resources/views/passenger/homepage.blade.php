...
@section('content')
    ...
    @if (session('success') || request('payment_success'))
        <div class="alert alert-success alert-dismissible fade show text-center" role="alert"
            ...
            <p class="mb-0">{{ session('success') ?: ('Payment successful! Your booking reference is: ' . request('payment_success')) }}</p>
        </div>
    @endif

    <!-- Centered Dropdown Error Alert (No Auto-dismiss) -->
    @if (session('error') || request('payment_error'))
        <div class="alert alert-danger alert-dismissible fade show text-center" role="alert"
            ...
            <p class="mb-0">{{ session('error') ?: 'Payment could not be completed. Please try again.' }}</p>
        </div>
    @endif

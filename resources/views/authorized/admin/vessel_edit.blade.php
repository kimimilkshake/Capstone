      ...
      <div class="form-row">
        ...
        <div class="form-col">
          <div class="form-group">
            ...
            <input type="number" id="vessel_total_passenger_capacity" name="vessel_total_passenger_capacity" value="{{ $vessel->vessel_total_passenger_capacity }}" min="1" required>
          </div>
        </div>
      </div>
              ...
              @foreach ($vessel->hatches as $index => $hatch)
                <div class="hatch-row">
                  ...
                  <div class="form-group hatch-input">
                    ...
                    <input type="number" name="hatches[{{ $index }}][length]" value="{{ $hatch->hatch_length }}" step="0.01" inputmode="decimal" required>
                  </div>
                  <div class="form-group hatch-input">
                    <label>Width (m)</label>
                    <input type="number" name="hatches[{{ $index }}][width]" value="{{ $hatch->hatch_width }}" step="0.01" inputmode="decimal" required>
                  </div>
                  <div class="form-group hatch-input">
                    <label>Height (m)</label>
                    <input type="number" name="hatches[{{ $index }}][height]" value="{{ $hatch->hatch_height }}" step="0.01" inputmode="decimal" required>
                  </div>
                  <div class="form-group hatch-input">
                    <label>Weight Capacity (%)</label>
                    <input type="number" name="hatches[{{ $index }}][weight_capacity]" value="{{ $hatch->hatch_weight_capacity }}" step="0.01" inputmode="decimal">
                  </div>
                  <div class="form-group hatch-input">
                    <label>Area Capacity (m³)</label>
                    <input type="number" name="hatches[{{ $index }}][area_capacity]" value="{{ $hatch->hatch_area_capacity }}" step="0.01" inputmode="decimal" required>
                  </div>
                  <div class="form-group hatch-input">
                    <label>Capacity per Hold (Tons)</label>
                    <input type="number" name="hatches[{{ $index }}][capacity_per_hold]" value="{{ $hatch->hatch_capacity_per_hold }}" step="0.01" inputmode="decimal" required>
                  </div>
          ...
          <div id="accommodation-container">
            @foreach ($vessel->accommodations as $index => $acc)
              <div class="accommodation-row">
                ...
                <div class="form-group acc-input">
                  ...
                  <input type="number" name="accommodations[{{ $index }}][price]" value="{{ $acc->accommodation_regular_price }}" step="0.01" inputmode="decimal" required>
                </div>
                <div class="form-group acc-input">
                  <label>Cot Range</label>
                  <input type="text" name="accommodations[{{ $index }}][cot_range]" value="{{ $acc->accommodation_cot_range }}" placeholder="Cot Range (e.g., 1-5, 7-10)" required>
                </div>
            ...
            @if ($vessel->accommodations->isEmpty())
              <div class="accommodation-row">
                ...
                <input type="text" name="accommodations[0][cot_range]" placeholder="Cot Range" required>
                <button type="button" class="accommodation-btn add-accommodation">+</button>
              </div>
            @endif
          </div>
      ...
      <div class="form-row" style="display: flex; gap: 2rem; align-items: flex-start; width: 100%;">
        <div class="form-col" style="flex: 1;">
          <div class="form-group vcot-plan" style="display: flex; flex-direction: column; align-items: flex-start; width: 100%;">
            ...
            @if ($vessel->vessel_cot_plan_url)
              <img src="{{ asset('storage/'.$vessel->vessel_cot_plan_url) }}" alt="Cot Plan" style="width: 100%; max-height: 250px; object-fit: contain; border: 1px solid #ccc; border-radius: 8px; margin-bottom: 10px;">
            @else
              <p style="color: #888; font-style: italic; margin-bottom: 10px;">No cot plan uploaded yet.</p>
            @endif
            <input type="file" id="vessel_cot_plan_url" name="vessel_cot_plan_url" accept="image/*" style="width: 100%; margin-top: 5px;">
          </div>
        </div>
        ...
        <div class="form-col" style="flex: 1;">
          <div class="form-group" style="width: 100%;">
            ...
            <select name="vessel_status" id="vessel_status" required style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 8px;">

    ...
    <div class="sms-container">
      <form method="POST" action="{{ route('staff.semaphore.send') }}" id="smsForm">
        ...
        <div class="sms-template-row">
          ...
          <button type="button" class="sms-template-button" data-template="Dear Passengers and Cargo Senders,\nDue to the impending arrival of Typhoon [Name] and the corresponding safety warnings issued by local authorities, we are regrettably cancelling the voyage from {{ optional($voyage->routePort)->route_origin ?? '-' }} to {{ optional($voyage->routePort)->route_destination ?? '-' }} scheduled for {{$voyage->voyage_departure_date}} to ensure the safety of our guests and staff.">Typhoon</button>
          <button type="button" class="sms-template-button" data-template="Dear Passengers and Cargo Senders,\nDue to an unexpected technical issue affecting our vessel, we are regrettably cancelling the voyage from {{ optional($voyage->routePort)->route_origin ?? '-' }} to {{ optional($voyage->routePort)->route_destination ?? '-' }} scheduled for {{$voyage->voyage_departure_date}} until the issue is resolved. We apologize for the inconvenience and will keep you updated.">Technical</button>
        </div>

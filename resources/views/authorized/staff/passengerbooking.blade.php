...
<div class="staff-body">
    ...
    <form action="{{ route('staff.passenger_booking.store') }}" method="POST" id="passengerBookingForm">
                ...
                <select name="voyage_id" id="voyageSelect" class="form-select" required>
                    ...
                    @foreach($voyages as $voyage)
                        <option value="{{ $voyage->voyage_id }}" 
                                data-vessel-id="{{ $voyage->vessel_id }}"
        ...
        <input type="hidden" name="payment_mode" value="Physical">

        <div class="text-center mt-4 mb-5">
            <button type="button" id="proceedPayment" class="btn btn-primary btn-lg" disabled>PROCEED TO PAYMENT</button>
            ...
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="paymentModalLabel">
                        <i class="fas fa-money-bill-wave"></i> Select Payment Mode
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="form-check mb-3 p-3 border rounded">
                        <input class="form-check-input" type="radio" name="modal_payment_mode" id="modalPaymentPhysical" value="Physical" checked>
                        <label class="form-check-label w-100" for="modalPaymentPhysical">
                            <strong><i class="fas fa-cash-register"></i> Physical (Cash)</strong>
                            <p class="small text-muted mb-0">Payment received in person. Booking will be immediately confirmed.</p>
                        </label>
                    </div>
                    <div class="form-check p-3 border rounded">
                        <input class="form-check-input" type="radio" name="modal_payment_mode" id="modalPaymentGcash" value="GCash">
                        <label class="form-check-label w-100" for="modalPaymentGcash">
                            <strong><i class="fas fa-mobile-alt"></i> GCash</strong>
                            <p class="small text-muted mb-0">Customer will pay online. Booking pending until payment confirmed.</p>
                        </label>
                    </div>
                </div>
...
function checkFormCompletion() {
    ...
    document.getElementById('proceedPayment').disabled = !allFieldsFilled;
}
...
document.getElementById('confirmBooking').addEventListener('click', function() {
    const selectedPaymentMode = document.querySelector('input[name="modal_payment_mode"]:checked').value;
    document.querySelector('input[name="payment_mode"]').value = selectedPaymentMode;

    ...
    <div class="sms-container">
      <form method="POST" action="<?php echo e(route('staff.semaphore.send')); ?>" id="smsForm">
        ...
        <div class="sms-template-row">
          ...
          <button type="button" class="sms-template-button" data-template="Dear Passengers and Cargo Senders,\nDue to the impending arrival of Typhoon [Name] and the corresponding safety warnings issued by local authorities, we are regrettably cancelling the voyage from <?php echo e(optional($voyage->routePort)->route_origin ?? '-'); ?> to <?php echo e(optional($voyage->routePort)->route_destination ?? '-'); ?> scheduled for <?php echo e($voyage->voyage_departure_date); ?> at <?php echo e($voyage->voyage_estimated_TD ? \Carbon\Carbon::parse($voyage->voyage_estimated_TD)->format('h:i A') : '-'); ?> to ensure the safety of our guests and staff." title="Typhoon Template"><i class="fas fa-cloud-showers-heavy"></i></button>
          <button type="button" class="sms-template-button" data-template="Dear Passengers and Cargo Senders,\nDue to an unexpected technical issue affecting our vessel, we are regrettably cancelling the voyage from <?php echo e(optional($voyage->routePort)->route_origin ?? '-'); ?> to <?php echo e(optional($voyage->routePort)->route_destination ?? '-'); ?> scheduled for <?php echo e($voyage->voyage_departure_date); ?> at <?php echo e($voyage->voyage_estimated_TD ? \Carbon\Carbon::parse($voyage->voyage_estimated_TD)->format('h:i A') : '-'); ?> until the issue is resolved. We apologize for the inconvenience and will keep you updated." title="Technical Template"><i class="fas fa-wrench"></i></button>
        </div>

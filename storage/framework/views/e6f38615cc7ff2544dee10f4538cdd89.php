      ...
      <div class="sms-modal-body">
        <form method="POST" action="<?php echo e(route('staff.semaphore.send')); ?>" id="smsForm">
          ...
          <div class="sms-template-row">
            ...
            <button type="button" class="sms-template-button template-typhoon" id="templateTyphoon" title="Typhoon Template" aria-label="Use typhoon template"><i class="fas fa-cloud-showers-heavy"></i></button>
            <button type="button" class="sms-template-button template-technical" id="templateTechnical" title="Technical Template" aria-label="Use technical issue template"><i class="fas fa-wrench"></i></button>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      templateTyphoon.addEventListener('click', function() {
        var text = "Dear Valued Passengers and Cargo Senders,\n\nDue to the impending arrival of Typhoon [Name] and the corresponding safety advisories issued by local authorities, we regret to inform you that the voyage from " + currentVoyageData.origin + " to " + currentVoyageData.destination + " scheduled for " + currentVoyageData.departureDate + " at " + currentVoyageData.etd + " has been cancelled in the interest of ensuring the safety and well-being of our passengers, cargo, and staff.";
        messageTextarea.value = text;
        messageTextarea.focus();
      });

      templateTechnical.addEventListener('click', function() {
        var text = "Dear Valued Passengers and Cargo Senders,\n\nDue to an unexpected technical issue affecting our vessel, we regret to inform you that the voyage from " + currentVoyageData.origin + " to " + currentVoyageData.destination + " scheduled for " + currentVoyageData.departureDate + " at " + currentVoyageData.etd + " has been cancelled until the issue is fully resolved. We sincerely apologize for any inconvenience caused and will provide updates as they become available.";
        messageTextarea.value = text;
        messageTextarea.focus();
      });
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Shem\Desktop\Capstone\resources\views/authorized/staff/svoyage_list.blade.php ENDPATH**/ ?>